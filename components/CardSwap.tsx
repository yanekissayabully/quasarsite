...
const CardSwap: React.FC<CardSwapProps> = ({
  ...
  const [responsiveSize, setResponsiveSize] = useState({ width, height });
    ...
    const updateSize = () => {
      ...
      if (mobile) {
        ...
        setResponsiveSize({
          ...
          height: Math.min(window.innerHeight * 0.4, 280)
        });
      } else if (tablet) {
        // Планшеты
        setResponsiveSize({
          width: Math.min(window.innerWidth * 0.6, 550),
          height: Math.min(window.innerHeight * 0.45, 350)
        });
      } else {
        // Десктоп
  ...
  const responsiveSkew = isMobile ? skewAmount * 0.7 : skewAmount;
  ...
  useEffect(() => {
    ...
    return () => clearInterval(intervalRef.current);
  }, [responsiveCardDistance, responsiveVerticalDistance, delay, pauseOnHover, responsiveSkew, easing, isMobile]);
