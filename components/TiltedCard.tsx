...
//       // Map to rotation values with enhanced amplitude
//       const rotationX = (normalizedBeta / 45) * rotateAmplitude
//       const rotationY = (normalizedGamma / 45) * rotateAmplitude
...
//       window.removeEventListener('deviceorientation', handleOrientation)
//     }
//   }, [enableGyroscope, isMobile, rotateX, rotateY, rotateAmplitude, isGyroActive])
...
//       onTouchStart={handleTouch}
//     >
//       {showMobileWarning && !isGyroActive && isMobile && !showPermissionBtn && (
//         <div className="absolute top-4 text-center text-xs px-4 text-white/70 z-10">
//           Наклоните устройство для 3D эффекта
//         </div>
//       )}

//       {showPermissionBtn && isMobile && (
...
export default function TiltedCard({
  ...
  useEffect(() => {
    ...
    const handleOrientation = (event: DeviceOrientationEvent) => {
      ...
      // Map to rotation values with enhanced amplitude
      const rotationX = (normalizedBeta / 45) * rotateAmplitude * 1.5
      const rotationY = (normalizedGamma / 45) * rotateAmplitude * 1.5
